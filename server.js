...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (data) => {
        ...
        // 파싱한 메시지의 타입을 체크한다.
        // 클라이언트에서 WebSocket 연결이 열린 이후 첫 전송한 참여자 정보 메시지 타입인 경우
                ...
                chatUsers.forEach((user) => {
                    ...
                    wss.clients.forEach((client) => {
                        ...
                        if (client._socket.remoteAddress === user.ip && client._socket.remotePort === user.port) {
                            // 가져온 클라이언트 정보에와 메시지 전송자의 정보에서 IP 주소와 포트 번호를 가져와 일치하지 않는지 체크한다.
                            ...
                            if (client._socket.remoteAddress !== sender.ip || client._socket.remotePort !== sender.port) {
                                // 수신한 Offer 메시지를 JSON 문자열로 변환하여 메시지 전송자를 제외한 채팅방 아이디에 해당하는 채팅방에 참여중인 클라이언트에 전송한다.
        ...
        // Answer 타입인 경우
            ...
            // 채팅방 정보가 존재하는 경우
            if (chatUsers) {
                // 메시지를 전송한 클라이언트의 IP 주소와 포트 번호를 가져온다.
                const sender = { ip: clientAddress, port: clientPort };
                ...
                chatUsers.forEach((user) => {
                    ...
                    wss.clients.forEach((client) => {
                        ...
                        if (client._socket.remoteAddress === user.ip && client._socket.remotePort === user.port) {
                            ...
                            // IP 주소와 포트 번호가 일치하지 않는 경우 - 메시지 전송자를 제외한 채팅방 아이디에 해당하는 채팅방에 참여중인 클라이언트
                            if (client._socket.remoteAddress !== sender.ip || client._socket.remotePort !== sender.port) {
                                // 수신한 Answer 메시지를 JSON 문자열로 변환하여 메시지 전송자를 제외한 채팅방 아이디에 해당하는 채팅방에 참여중인 클라이언트에 전송한다.
                ...
                chatUsers.forEach((user) => {
                    ...
                    wss.clients.forEach((client) => {
                        ...
                        if (client._socket.remoteAddress === user.ip && client._socket.remotePort === user.port) {
                            ...
                            // IP 주소와 포트 번호가 일치하지 않는 경우 - 메시지 전송자를 제외한 채팅방 아이디에 해당하는 채팅방에 참여중인 클라이언트
                            if (client._socket.remoteAddress !== sender.ip || client._socket.remotePort !== sender.port) {
                                // 수신한 ICE Candidate 정보 메시지를 JSON 문자열로 변환하여 메시지 전송자를 제외한 채팅방 아이디에 해당하는 채팅방에 참여중인 클라이언트에 전송한다.
        ...
        // 클라이언트에서 WebSocket 연결이 닫힌 이후 전송한 퇴장 메시지 타입인 경우
                ...
                chatUsers.forEach((user) => {
                    ...
                    wss.clients.forEach((client) => {
                        ...
                        if (client._socket.remoteAddress === user.ip && client._socket.remotePort === user.port) {
                            ...
                            // IP 주소와 포트 번호가 일치하지 않는 경우 - 메시지 전송자를 제외한 채팅방 아이디에 해당하는 채팅방에 참여중인 클라이언트
                            if (client._socket.remoteAddress !== sender.ip || client._socket.remotePort !== sender.port) {
                                // 수신한 WebSocket 연결 종료 메시지를 JSON 문자열로 변환하여 메시지 전송자를 제외한 채팅방 아이디에 해당하는 채팅방에 참여중인 클라이언트에 전송한다.
    ...
    // WebSocket 연결이 닫힐 때 실행되는 이벤트 핸들러 - 연결된 클라이언트가 단 한명도 없는 경우
    ws.on('close', () => {
        // 채팅방 아이디에 해당하는 채팅방 정보를 가져온다.
        const chatUsers = chatRooms[message.id];
            ...
            chatUsers.forEach((user) => {
                ...
                wss.clients.forEach((client) => {
                    ...
                    if (client._socket.remoteAddress === user.ip && client._socket.remotePort === user.port) {
                        ...
                        // IP 주소와 포트 번호가 일치하지 않는 경우 - 메시지 전송자를 제외한 채팅방 아이디에 해당하는 채팅방에 참여중인 클라이언트
                        if (client._socket.remoteAddress !== sender.ip || client._socket.remotePort !== sender.port) {
                            // WebSocket 연결 종료 메시지를 생성한다.
                            let exit = { type: 'getExit', data: user.idx };
                            // 생성한 WebSocket 연결 종료 메시지를 JSON 문자열로 변환하여 메시지 전송자를 제외한 채팅방 아이디에 해당하는 채팅방에 참여중인 클라이언트에 전송한다.
